...
def print_error(error_index: int, error: HexagonalError):
    ...
    print(f'[ERROR {error_index + 1}] Onion Architecture: {error.message}')
...
def run_command(command: str):
    if command == 'diagram':
        ...
        onion_check = HexagonalSanityCheck()
        errors = onion_check.check('src/')

        [print_error(index, error) for index, error in enumerate(errors)]
        if len(errors) > 0:
            raise Exception('Onion Architecture: Errors found in dependencies flow')

        print('Onion Architecture: No errors found')
    else:
